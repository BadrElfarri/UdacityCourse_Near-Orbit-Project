...
class AttributeFilter:
    ...
    def __init__(self, op, value):
        ...
        Constructed from a binary predicate and a reference value.
    ...
    # callables - if we have an instance of a
    # subclass of AttributeFilter named f, then the code f(approach) is
    # really evaluating f.__call__(approach).
...
class HazardousFilter(AttributeFilter):
    ...
    @classmethod
    def get(cls, approach):
        """Override the superClass methode."""
...
def limit(iterator, n=None):
    ...
    if n is None or n == 0:
        ...
        # 'slice' the iterator to stop after yielding n times using a own
        # defined 'iterator'
        it = iter(range(0, n, 1))
        nexti = next(it)
        try:
            # enumrate returns (index, element)
